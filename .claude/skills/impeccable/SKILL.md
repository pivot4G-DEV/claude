...
> Vendored from [pbakaus/impeccable](https://github.com/pbakaus/impeccable) (Apache 2.0 license). See `LICENSE` in this directory for the original license terms. Some reference workflows (e.g. `live`) may expect the `impeccable` npm CLI (`npx impeccable ...`), which is fetched from npm on demand, not vendored here.
